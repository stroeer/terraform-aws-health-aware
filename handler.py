...
def send_alert(event_details, affected_accounts, affected_entities, event_type):
    ...
    if "None" not in event_bus_name:
        try:
            ...
            send_to_eventbridge(get_detail_for_eventbridge(event_details, affected_entities), event_type, resources,
                                event_bus_name, eventbus_endpoint)
    ...
    if "None" not in event_bus_name:
        try:
            ...
            send_to_eventbridge(
                ...
                event_type, resources, event_bus_name, eventbus_endpoint)
        except HTTPError as e:
            print("Got an error while sending message to EventBridge: ", e.code, e.reason)
        except URLError as e:
            print("Server connection failed: ", e.reason)
    ...
    for account in affected_accounts:
        ...
        for event_entities_page in event_entities_page_iterator:
            ...
            for entity in parsed_event_entities['entities']:
                entity.pop(
                    "entityArn")  # remove entityArn to avoid confusion with the arn of the entityValue (not present)
    ...
    for response in event_page_iterator:
        ...
        if len(aws_events) > 0:  # if there are new event(s) from AWS
            for event in aws_events:
                ...
                affected_entities = get_affected_entities(health_client, event_arn, affected_accounts,
                                                          is_org_mode=False)
            ...
            for event in aws_events:
                ...
                if os.environ['ACCOUNT_IDS'] == "None" or os.environ['ACCOUNT_IDS'] == "":
                    ...
                    if affected_org_accounts != []:
                        focused_org_accounts = [i for i in affected_org_accounts if i in account_ids_to_filter]
                ...
                affected_org_entities = get_affected_entities(health_client, event_arn, affected_org_accounts,
                                                              is_org_mode=True)
                # get event details
...
def getAccountIDs():
    ...
    return os.environ['ACCOUNT_IDS'].split(",")
